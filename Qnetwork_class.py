...
class Qnetwork():
	...
	def __init__(self,n_actions,learning_rate):
		# placeholder shape == (None,4) for the states; #state: x,y position, x,y velocity
		self.inputs = tf.placeholder(shape=[None,4],dtype=tf.float32,name="Input_States")
		...
		# default activation_fn is Relu
		hidden1 = layers.fully_connected(inputs=self.inputs,num_outputs=64,biases_initializer=None)
		...
		# dropout?
		# 8 action in output, no activation == linear layer, to negative values to be generated
		self.Q_outlayer = layers.fully_connected(inputs=hidden5,num_outputs=n_actions,activation_fn=None,biases_initializer=None)
		...
		self.actions = tf.placeholder(shape=[None],dtype=tf.int32,name="Actions_holder")
		self.actions_onehot = tf.one_hot(self.actions,8,dtype=tf.float32)

		# calculating loss by taking the sum of of squares differences between
		# the Q function's TD-target and the actual Q values
		# get Q values,  reduction_indicies == axis
		self.Q = tf.reduce_sum(tf.multiply(self.Q_outlayer, self.actions_onehot), reduction_indices=1)
		# targetQ values:
