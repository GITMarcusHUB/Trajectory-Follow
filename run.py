	...
	with tf.Session() as sess:
		...
		while i < hparams.num_episodes+1:
			...
			while j <  hparams.num_steps+1:
				j+=1
				if (np.random.randn(1) < epsilon or total_steps < hparams.pre_train_steps ):#or total_steps < hparams.buffer_size):
					...
					action,Q_all = sess.run(fetches=[q_net.predict,q_net.Q_outlayer],feed_dict={q_net.inputs : [state],q_net.keep_per : 1.0}) 
					action=action[0]
					#print("Action:",action)
				...
				if (total_steps > hparams.pre_train_steps and total_steps % hparams.train_freq == 0) and total_steps > hparams.buffer_size:
					trainBatch = replay_buffer.sample(hparams.batch_size) #prioritization: samoling with probability relative to the last encountered absolute TD-error
					Q1 = sess.run(fetches=q_net.predict,feed_dict={q_net.inputs : np.vstack(trainBatch[:,3]),q_net.keep_per : 1.0})
					Q2 = sess.run(fetches=target_net.Q_outlayer,feed_dict={target_net.inputs : np.vstack(trainBatch[:,3]),target_net.keep_per : 1.0})
					doubleQ = Q2[range(hparams.batch_size),Q1]
					targetQ = trainBatch[:,2] + (hparams.gamma* doubleQ * -(trainBatch[:,4] - 1 ))
					_,updateloss,updateerr = sess.run(fetches=[q_net.updateQ,q_net.loss,q_net.err],feed_dict={q_net.inputs : np.vstack(trainBatch[:,0]),q_net.targetQ : targetQ, q_net.keep_per : 1.0, q_net.actions : trainBatch[:,1]})
					update_target(target_ops,sess)
