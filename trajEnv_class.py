...
class traj_env():
	def __init__(self,PMDP,sizeX,sizeY,epsilon):
		...
		# to add some extra space to move for the agent, this may cause problems
		self.sizeX=self.sizeX - int((self.sizeX-coordsX[-1])) +10
		...
		self.agent=agent(posx=self.trajectory.coordinatesX[0], posy=self.trajectory.coordinatesY[0],
						 reward=0.0, velocity=np.zeros(2))
	...
	# agent will learn from 8 differenct accelerating actions
	def accelerate_agent(self,direction):
	...
	def nearest_traj_idx(self):
		...
		for i in range(self.trajectory.coordinatesX.shape[0]):
			...
			if np.add(
					np.square(agentPos[0]-trajX[i]), np.square(agentPos[1]-trajY[i])
			) < np.add(
				np.square(agentPos[0]-trajX[ind]), np.square(agentPos[1]-trajY[ind])
			):
				ind = i
	...
	def check_goal(self):
		if self.agent.get_pos()[0] == self.trajectory.get_goal()[0] and \
				(self.agent.get_pos()[1] >= self.trajectory.get_goal()[1]-self.epsilon) and \
				(self.agent.get_pos()[1] <= self.trajectory.get_goal()[1]+self.epsilon):
		...
		for i in range(self.trajectory.checkpoints.shape[0]):
			if self.agent.max_x < self.trajectory.checkpoints[i][0]:
				if(self.agent.X == self.trajectory.checkpoints[i][0] and 
					...
					or (self.agent.Y > self.trajectory.checkpoints[i][1]-(self.epsilon/2) and
						self.agent.Y < self.trajectory.checkpoints[i][1]+(self.epsilon/2) ))):
...
def control_agent(self, direction):
		...
		if direction == 2 and self.agent.X<=self.sizeX-2:
			self.agent.X+=1
		if direction == 3 and self.agent.Y <= self.sizeY-2 and self.agent.X<=self.sizeX-2:
			self.agent.Y+=1
			self.agent.X+=1
